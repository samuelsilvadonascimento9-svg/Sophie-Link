...
switch ($method . ':' . $action) {
    ...
    // GET /app/api/empresas/index.php?action=listar
    case 'GET:listar':
    ...
    // GET /app/api/empresas/index.php?action=buscar&id=X
    case 'GET:buscar':
        $id = (int)($_GET['id'] ?? 0);
    ...
    // POST /app/api/empresas/index.php?action=criar
    case 'POST:criar':
    ...
    // POST /app/api/empresas/index.php?action=atualizar&id=X
    case 'POST:atualizar':
        $id    = (int)($_GET['id'] ?? 0);
        $dados = json_decode(file_get_contents('php://input'), true) ?? $_POST;
        if (empty($dados['nome'])) jsonResponse(false, null, 'O campo Nome é obrigatório.', 422);
    ...
    // POST /app/api/empresas/index.php?action=excluir&id=X
    case 'POST:excluir':
        $id = (int)($_GET['id'] ?? 0);
        $ok = $model->excluir($id);
